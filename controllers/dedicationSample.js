...
const sanitize = (doc) => ({ id: doc._id, type: doc.type, video: doc.video, userId: doc.userId, createdAt: doc.createdAt, updatedAt: doc.updatedAt });
...
export const createDedicationSample = async (req, res) => {
  try {
    ...
    const created = await DedicationSample.create({ type: type.trim(), video: String(video).trim(), userId: req.user._id });
